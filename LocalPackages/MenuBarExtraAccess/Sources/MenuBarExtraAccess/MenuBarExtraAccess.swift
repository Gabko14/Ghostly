...
extension Scene {
    ...
    /// - Parameter staysOpen: When `true`, the window will not auto-close when it loses key status
    ///   (e.g. when the user switches to another app). The window can still be dismissed explicitly
    ///   by toggling the status item or by setting `isPresented` to `false`.
    public func menuBarExtraAccess(
        index: Int = 0,
        isPresented: Binding<Bool>,
        isEnabled: Binding<Bool> = .constant(true),
        staysOpen: Binding<Bool> = .constant(false),
    ...
    private func observerSetup() -> Int {
        ...
        observerContainer.setupWindowObservers(
            ...
            didResignKey: { window in
                ...
                if staysOpen {
                    if window.isVisible {
                        // Window lost focus but pin mode is active — keep it open.
                        return
                    } else {
                        // Window was closed by other means (e.g. status item click) — update state.
